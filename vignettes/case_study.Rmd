...
# Time points to numeric
colData(mae_temp)[["timepoint"]] <- ifelse(
    colData(mae_temp)[["timepoint"]] == "start", 1, 2)

# Transform MAE object to MOFA model
model <- create_mofa_from_MultiAssayExperiment(
    mae_temp, extract_metadata = TRUE)
# Add temporal covariate
model <- set_covariates(model, covariates = "timepoint")
