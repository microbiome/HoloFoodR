...
hist(rowData(mae[[1]])[["sd"]], breaks = 30, main = "", xlab = "Standard deviation")
...
```{r}
cat("Proportion of fatty acids with standard deviation below 0.5 is", round(mean(rowData(mae[[1]])[["sd"]] < 0.5), 3), "\n")

cat("Proportion of fatty acids with standard deviation below 1 is", round(mean(rowData(mae[[1]])[["sd"]] < 1), 3))
```
...
Factor 2, no single fatty acid can be directly associated with any of the microbial species, including _Photobacterium_.
...
```{r, fig.height = 6, fig.width = 12, paper_figure}

# Extract omage-6 fatty acids
dha = rownames(tse)[1]
epa = rownames(tse)[2]

# Abundance plot of microbes by study group and time point
p_abundance <- plotAbundance(
    altExp(mae[[2]], "prev_genus"),
    assay.type = "relabundance",
    col.var = c("study_group", "timepoint"),
    facet.cols = TRUE, scales = "free_x"
...
# Expression plots
p2_dha <- plotExpression(
    tse, dha, assay.type = "counts", x = "timepoint",
    colour_by = "study_group", scales = "free")
p3_epa <- plotExpression(
    tse, epa, assay.type = "counts", x = "timepoint",
    colour_by = "study_group", scales = "free")

(p_abundance + p2_dha / p3_epa) 
```
